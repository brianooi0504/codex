...
def multiple_subscriptions(entity_type_flag, watched_attributes_flag, entity_id_flag, area, context, truetype, true_id, expires, broker, port, qos, watched_attributes, server_client_flag, queue):
    topic = []
    ...
    subscribe(broker, port, topic, expires, qos, server_client_flag, queue, context_given=context)
...
def subscribe(broker, port, topics, expires, qos, server_client_flag, queue, context_given):
    run_flag = True
    ...
    def on_message(client, userdata, msg):
        ...
        if msg.payload.decode() != '':
            ...
            if server_client_flag == 1:
                # Server code to handle received client data
                print('\nid: ', data2['value'][0])
                print('type: ', data2['value'][1])
                print('modelName: ', data2['value'][2])
                print('modelType: ', data2['value'][3])
                print('modelWeightSizes: ', data2['value'][4])
                print('modelIter: ', data2['value'][5])
                print('clientDataPoints: ', data2['value'][6])
                print('modelWeights: ', data2['value'][7])
                print()
                received_client_model = {'id': data2['value'][0], 'type': data2['value'][1], 'modelName': data2['value'][2], 'modelType': data2['value'][3], 'modelWeightSizes': data2['value'][4], 'iteration': data2['value'][5], 'datapoints': data2['value'][6], 'weights': data2['value'][7]}
                queue.put(received_client_model)
            elif server_client_flag == 2:
                # Client code to handle received server data
                print('\nid: ', data2['value'][0])
                print('type: ', data2['value'][1])
                print('modelName: ', data2['value'][2])
                print('modelType: ', data2['value'][3])
                print('modelWeightSizes: ', data2['value'][4])
                print('modelIter: ', data2['value'][5])
                print('modelWeights: ', data2['value'][6])
                print()
                received_server_model = {'id': data2['value'][0], 'type': data2['value'][1], 'modelName': data2['value'][2], 'modelType': data2['value'][3], 'modelWeightSizes': data2['value'][4], 'iteration': data2['value'][5], 'weights': data2['value'][6]}
                queue.put(received_server_model)
...
def subscribe_for_advertisement_notification(broker, port, topics, expires, qos, entity_type_flag, watched_attributes_flag, entity_id_flag, watched_attributes, true_id, server_client_flag=0, queue=None):
    run_flag = True
            ...
            for i in range(0, number_of_threads):
                ...
                process = multiprocessing.Process(
                    ...
                    args=(
                        ...
                        context_providers_addresses[i], int(context_providers_ports[i]), qos, watched_attributes, server_client_flag, queue
                    )
                )
                jobs.append(process)
...
def train_local_model(model):
    num_epochs = 10
    learning_rate = 0.001
    num_batches = 4
    
    mini_batches = np.array_split(model.dataset, num_batches)

    for epoch in range(num_epochs):
        for batch in range(num_batches):
            X_train = mini_batches[batch][:,:-1]
            y_train = mini_batches[batch][:,-1]

            m,n = X_train.shape
            pred = model.forward(X_train)
            
            cost = 1/2 * (mean_squared_error(pred, y_train)) # Calculate cost function
            ...
            for t_cols in range(0,n): 
                t = theta[t_cols] - learning_rate/m * sum((pred-y_train)*X_train[:,t_cols]) # calculate new theta value
                theta_new.append(t)

            model.fc = np.array(theta_new)

    return model, cost

def read_ngsild_file(file_path):
    try:
        with open(file_path, 'r') as file:
            ngsild_data = json.load(file)
        return ngsild_data
...
def write_ngsild_file(file_path, ngsild_data):
    try:
        with open(file_path, 'w') as file:
            json.dump(ngsild_data, file, indent=2)
...
def main(argv):
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-fr", "--client_fraction"):
            client_fraction = float(arg)
    ...
    #FL SERVER               
        ...
        if file=='' or data_file=='':
            usage()
            sys.exit(2)

        if (client_fraction > 1):
            print("Invalid fraction of clients (0 - 1)")
            sys.exit(2) 

        if (num_rounds < 1):
            print("Invalid number of rounds (integer >= 0)")
            sys.exit(2)
        ...
        for i in range(num_rounds):
            ...
            print("Server round: ", server_training_iteration)
            ...
            if len(client_models) > 0:
                ...
                for client_model in client_models:
                    # if client_model['iteration'] == server_training_iteration:
                    total_datapoints += client_model['datapoints']

                print('Total datapoints: ', total_datapoints)

                for client_model in client_models:
                    # if client_model['iteration'] == server_training_iteration:
                    aggregated_model.fc += (client_model['datapoints'] / total_datapoints) * np.array(client_model['weights'])

                print('global', global_model.fc)
                print('agg', aggregated_model.fc)
                    
                # Model averaging
                global_model.fc = aggregated_model.fc

                # EVALUATE MODEL
                prediction = global_model.forward(data_test[:,:-1])
                error = mean_squared_error(prediction,data_test[:,-1])
                costs.append(error)
                print(error)
    ...
    #FL CLIENT               
        ...
        for i in range(num_rounds):
            print("Client round:", clientModelIteration)
